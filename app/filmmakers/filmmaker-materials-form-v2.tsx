...
const DRAFT_KEY = "filmshow-filmmaker-materials-draft-v2";
...
function Question({
  ...
  return (
    <section className={`filmmaker-question ${className}`} data-reveal="text">
      <div className="filmmaker-question-number" aria-hidden="true">{number}</div>
      <div className="filmmaker-question-body">
        ...
        {children}
        {error ? <p className="filmmaker-field-error" role="alert">{error}</p> : null}
      </div>
    </section>
  );
}

export function FilmmakerMaterialsFormV2() {
  ...
  useEffect(() => {
    const restoreDraft = window.setTimeout(() => {
      try {
        ...
        if (savedDraft) {
          const draft = JSON.parse(savedDraft) as Partial<FormValues> & { subtitleStatus?: string };
          setValues({
            ...initialValues,
            ...draft,
            subtitleStatus:
              draft.subtitleStatus === "no_subtitles" || draft.subtitleStatus === "burned_in_master"
  ...
  const validate = () => {
    ...
    if (!values.showDayContact.trim()) nextErrors.showDayContact = "Give us a show-day name and cell number.";

    setErrors(nextErrors);
    const firstError = Object.keys(nextErrors)[0] as keyof FormValues | undefined;
    if (firstError) {
      window.setTimeout(() => {
        document.getElementById(focusIds[firstError] || `filmmakers-${firstError}`)?.focus();
      }, 0);
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setStatusMessage(result.message || "Something broke. Your answers are still here. Try SEND IT again.");
        return;
      }
      try {
      ...
      setStatusMessage("The connection dropped. Your answers are still here. Try SEND IT again.");
    } finally {
  ...
  return (
    <form className="filmmakers-form" onSubmit={handleSubmit} noValidate>
      <div className="filmmakers-container">
        <input className="filmmakers-honeypot" type="text" name="company" tabIndex={-1} autoComplete="off" aria-hidden="true" />

        <Question number="01" title="What's your email?" error={errors.email}>
          <input id="filmmakers-email" type="email" autoComplete="email" inputMode="email" value={values.email} onChange={(e) => setField("email", e.target.value)} aria-invalid={Boolean(errors.email)} placeholder="you@somewhere.com" />
        </Question>
        ...
        <section className="filmmakers-submit" data-reveal="text">
          {statusMessage ? <p className="filmmakers-status" role="alert">{statusMessage}</p> : null}
          <button type="submit" disabled={isSubmitting}>{isSubmitting ? "Sending..." : "Send it ->"}</button>
