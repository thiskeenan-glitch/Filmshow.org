...
begin
  ...
  on conflict (email_normalized) do update set
    ...
    marketing_status = case
      when p_marketing_opt_in then 'subscribed'
      else public.audience_contacts.marketing_status
    end,
    marketing_consent_at = case
      when p_marketing_opt_in then now()
      else public.audience_contacts.marketing_consent_at
    end,
    marketing_consent_source = case
      when p_marketing_opt_in then nullif(btrim(p_consent_source), '')
      else public.audience_contacts.marketing_consent_source
    end,
    marketing_sync_status = case
      when p_marketing_opt_in then 'pending'
      else public.audience_contacts.marketing_sync_status
    end,
    marketing_last_error = case
      when p_marketing_opt_in then null
      else public.audience_contacts.marketing_last_error
    end,
    last_seen_at = now()
